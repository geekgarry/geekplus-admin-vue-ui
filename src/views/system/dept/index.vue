<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="status" label="状态" align="center" :formatter="statusFormat" width="100"></el-table-column>
...
<script>
import { listDept, getDept, delDept, addDept, updateDept,deptTreeSelect, listDeptExcludeChild } from "@/api/system/dept";
  ...
  methods: {
    ...
    handleAdd(row) {
      ...
      listDept().then(response => {
          ...
          //this.deptCascaderOptions=this.handleCascaderTree(this.deptOptions);
      });
    },
    handleCascaderTree(list){
      let deptOptions=[];
      list.forEach(item => {
    ...
    handleUpdate(row) {
      this.reset();
      let queryParams={deptId: row.deptId}
      getDept(queryParams).then(response => {
